...
func (m Model) View() string {
	...
	for i, row := range m.Rows {
		...
		state := row.State
		lines = append(lines, fmt.Sprintf("%s%s · %s [%s] dirty:%d +%d/-%d", prefix, platform.SafeText(row.Repository.Name), platform.SafeText(row.Branch), state, row.Dirty, row.Ahead, row.Behind))
