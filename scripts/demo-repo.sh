...
git -C "$root" add --all --
# The fixture is synthetic and must remain runnable on hosts that require
# unavailable signing providers. Release signing is validated separately by
# the signed-release workflow and must not leak into disposable test repos.
git -C "$root" -c commit.gpgsign=false commit -q -m 'demo baseline'
