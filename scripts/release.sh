...
for target in darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64; do
	...
	GOOS="$goos" GOARCH="$goarch" CGO_ENABLED=0 go build -trimpath -ldflags "-s -w -X github.com/sphireinc/git-watch/internal/version.Version=$version" -o "$out/gitwatch${suffix}" ./cmd/gitwatch
