...
Progress: Added explicit fetch and pull command primitives with merge, rebase, and fast-forward-only strategy options, rejecting unspecified strategies and option-like targets. The remotes workspace now fetches the selected remote asynchronously and refreshes status/remote metadata afterward; pull strategy selection, progress detail, and conflict UI remain.
