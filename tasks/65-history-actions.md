...
Progress: Added explicit-target checkout, branch-at-commit, tag listing, SHA-confirmed revert primitives, and option-like target rejection. The History workspace now requires explicit `x` then `y` confirmation naming the selected commit before detached checkout, and provides `B` branch-name entry for creating a branch at the selected commit; both successful actions refresh status. Tag navigation, revert UI, and end-to-end mutation coverage remain.
